...
function useWordle() {
  ...
  useEffect(() => {
    if (!state.endgame) {
      dispatch({ type: 'SET_NEW_RANDOM_WORD', payload: lettersArray[Math.floor(Math.random() * 2315) + 1] });
    }
  }, [state.endgame]);
  ...
  const addWord = (e) => {
    ...
    if (!state.error && !state.endgame) {
      if ((e.keyCode === 13 || e === 'ENTER') && state.numberOfGuess < 6 && !state.error) {
        ...
        //check the word in the wordlist
        if (!lettersArray.includes(state.currentGuess)) {
          addError('Not in word list');
          return;
        }
